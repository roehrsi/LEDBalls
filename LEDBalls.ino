...
#define THRESHOLD_SAMPLE_SIZE 20                                          // For calibrating the thresholds
#define THRESHOLD_FRAC 5                                                  // Set this to the fraction of the calibrated capacitance at which to fire a touch input, e.g. "3" for firing at dropping 1/3rd of cap value.
...
// Init target palettes to a random selection from predifined palettes. This will blend them in smoothly on startup.
...
void setup() {
  ...
  // Set the global brightness. Will not be used de to multiple controller setup above.
  FastLED.setBrightness(BRIGHTNESS);

  // Calibrate the touch sensor thresholds for each pin.
...
void loop() {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    if (touchSensorValues[i] > 0 && touchSensorValues[i] <= touchSensorThresholds[i]) {  // Check thresholds. Disregard zeroes
